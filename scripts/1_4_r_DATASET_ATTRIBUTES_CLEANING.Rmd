...
#### Using SDAM package (2021 version)
...
```{r, echo = FALSE }
resp = request("EDH_merged_2022-11-02.json", path="/sharingin/648597@au.dk/SDAM_root/SDAM_data/EDH/public", method="GET", cred=c(user, getPass("your sciencedata password: ")))
```
...
```{r}
list_json <- rjson::fromJSON(file="~/Projects/EDH_ETL_repo/data/large_data/EDH_merged.json") # , flatten=TRUE)
...
```{r}
EDH <- jsonlite::fromJSON("https://sciencedata.dk/public/b6b6afdb969d378b70929e86e58ad975/EDH_merged_2022-11-02.json")
```

Or load local dataset if you have it
```{r}
EDH <- jsonlite::fromJSON("../data/large_data/EDH_merged_2022-11-02.json")
```
...
ggplot(insc_typology_api, aes(x=n, y=type_of_inscription)) + geom_col(aes(fill=n)) + geom_point()
ggplot(insc_typology_xml, aes(x=n, y=keywords_term_text)) + geom_col(aes(fill=n)) + geom_point()
...
write(EDH_cleaned_json, file="data/large_data/EDH_attrs_cleaned_2022-11-03.json")
...
```{r}
request("data/large_data/EDH_attrs_cleaned_2022-11-03.json", path="/sharingout/648597@au.dk/SDAM_root/SDAM_data/EDH/public",
